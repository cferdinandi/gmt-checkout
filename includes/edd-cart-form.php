<?php
	 ...
	 */
	function gmt_edd_cart_form_is_stripe_checkout_class ( $classes ) {
		if ( function_exists( 'edd_get_option' ) && !empty( edd_get_option( 'stripe_checkout' ) ) ) {
			$classes[] = 'edd-stripe-checkout';
		}
		return $classes;
	}
	add_filter( 'body_class', 'gmt_edd_cart_form_is_stripe_checkout_class' );
	 ...
	 */
	function gmt_edd_cart_form_disable_verification_email () {
		remove_action( 'edd_send_verification_email', 'edd_process_user_verification_request' );
	}
	add_action('init', 'gmt_edd_cart_form_disable_verification_email');
	 ...
	 */
	function gmt_edd_cart_form_edd_add_powered_by_stripe( $label, $gateway ) {
		if ($gateway === 'stripe') {
			return edd_get_option('gmt_edd_custom_credit_card_label');
		}
		return $label;
	}
	add_filter( 'edd_gateway_checkout_label', 'gmt_edd_cart_form_edd_add_powered_by_stripe' );
	 ...
	 */
	function gmt_edd_cart_form_add_via_stripe ( $gateways ) {
	...
	add_filter( 'edd_payment_gateways', 'gmt_edd_cart_form_add_via_stripe' );
	 ...
	 */
	function gmt_edd_cart_form_edd_purchase_form_required_fields ( $required_fields ) {
		unset( $required_fields['edd_first'] );
		unset( $required_fields['edd_last'] );
		return $required_fields;
	}
	add_filter( 'edd_purchase_form_required_fields', 'gmt_edd_cart_form_edd_purchase_form_required_fields' );
	 ...
	 */
	function gmt_edd_cart_form_edd_remove_names () {
		remove_action( 'edd_purchase_form_after_user_info', 'edd_user_info_fields' );
	}
	add_action( 'init', 'gmt_edd_cart_form_edd_remove_names' );
	 ...
	 */
	function gmt_edd_cart_form_edd_user_info_fields () {
	...
	add_action( 'edd_purchase_form_after_user_info', 'gmt_edd_cart_form_edd_user_info_fields' );
	 ...
	 */
	function gmt_edd_cart_form_edd_remove_credit_card_validator () {
		wp_dequeue_script( 'creditCardValidator' );
	}
	add_action( 'wp_enqueue_scripts', 'gmt_edd_cart_form_edd_remove_credit_card_validator' );
	 ...
	 */
	function gmt_edd_cart_form_edd_no_js_disable_purchase () {
		...
		echo
			'<div id="gmt_edd_cart_form-edd-no-js-purchase-message">' .
				'<em>' . __( 'Please enabled JavaScript to complete your purchase.', 'gmt_edd_cart_form' ) . '</em><br>' .
				'<button class="btn btn-large disabled" disabled="disabled">' . $complete_purchase . '</button>' .
			'</div>';
	}
	add_action( 'edd_purchase_form_after_submit', 'gmt_edd_cart_form_edd_no_js_disable_purchase' );
	 ...
	 * GDPR Message
	 */
	function gmt_edd_cart_form_edd_gdpr_message () {
		echo edd_get_option('gmt_edd_custom_gdpr_message');
	}
	add_action( 'edd_purchase_form_after_submit', 'gmt_edd_cart_form_edd_gdpr_message' );
	 ...
	 */
	function gmt_edd_cart_form_edd_checkout_submit () {
	?>
	...
	add_action( 'edd_purchase_form_after_cc_form', 'gmt_edd_cart_form_edd_checkout_submit', 9999 );



	/**
	 * Remove description from checkout page
	 */
	function gmt_edd_cart_form_edd_remove_checkout_submit () {
		remove_action( 'edd_purchase_form_after_cc_form', 'edd_checkout_submit', 9999 );
	}
	add_action( 'init', 'gmt_edd_cart_form_edd_remove_checkout_submit' );
	 ...
	 */
	function gmt_edd_cart_form_edd_add_cart_link_to_nav ( $items, $args ) {
